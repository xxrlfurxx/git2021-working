...
public class AirService {
	...
	public void requestAirSiGunGuHour(String sido) throws IOException {
		...
		builder.append("/getCtprvnMesureSidoLIst"); // 기능(시도-시군구별조회 예)
													// 서울-강남구...중랑구)
		builder.append("?sidoName=" + URLEncoder.encode(sido, "UTF-8")); // 시도(서울,
																			// 경기...)
		...
		AirSigunguHourResponse response = new Gson().fromJson(json,
				AirSigunguHourResponse.class);
		System.out.println(response);

		// // 강동구 데이터
		// AirSigunguHourResponse.Item item =
		// response.getResponse().getBody().getItems().getItem().get(1);
		...
		for (AirSigunguHourResponse.Item item : response.getResponse().getBody()
				.getItems().getItem()) {
			AirSigunguHour record = AirSigunguHour.builder()
					.dataTime(item.getDataTime()).sidoName(item.getSidoName())
					.cityName(item.getCityName())
					.pm10Value(item.getPm10Value().isEmpty()
							? null
							: Integer.valueOf(item.getPm10Value()))
					.pm25Value(item.getPm25Value().isEmpty()
							? null
							: Integer.valueOf(item.getPm25Value()))
					.build();
