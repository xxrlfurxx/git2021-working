...
public class PhotoController {
	...
	private SortedMap<Long, Photo> photos = Collections.synchronizedSortedMap(
			new TreeMap<Long, Photo>(Collections.reverseOrder()));
	...
	public Photo addPhoto(@RequestBody Photo photo, HttpServletResponse res)
		...
		Photo photoItem = Photo.builder().id(currentId).title(photo.getTitle())
				...
				.photoUrl(photo.getPhotoUrl()).fileType(photo.getFileType())
				.fileName(photo.getFileType()).createdTime(new Date().getTime())
				.build();
	...
	public Photo modifyPhotos(@PathVariable long id, @RequestBody Photo photo,
		...
		photoItem.setDescription(
				TextProcesser.getPlainText(photo.getDescription()));
